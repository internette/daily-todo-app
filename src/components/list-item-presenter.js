...
const ListItemPresenter = ( props ) => {
  ...
  const editIcon = props.editable ? <div className="edit-icons"><a className="save-icon" onClick={props.saveUpdate}></a><a className="cancel-icon" onClick={props.cancel}></a></div> : <div className="edit-icons"><a className="edit-icon" onClick={props.toggleEdit}></a></div>
  ...
  const details_elm = props.details ? <div className={ props.editable ? 'details editing' : 'details'}>
                                  <Scrollbars style={{ width: '100%', height: "8rem" }}
